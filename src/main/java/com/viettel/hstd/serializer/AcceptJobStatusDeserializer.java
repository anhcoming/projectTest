...
public class AcceptJobStatusDeserializer extends JsonDeserializer<AcceptJobStatus> {

    @Override
    public AcceptJobStatus deserialize(JsonParser jp, DeserializationContext ctxt)
            throws IOException {
        ObjectCodec oc = jp.getCodec();
        IntNode node = oc.readTree(jp);
        int genderString = node.numberValue().intValue();

        return AcceptJobStatus.of(genderString);
    }
}
