...
public class ResignTypeDeserializer extends JsonDeserializer<ResignType> {

    @Override
    public ResignType deserialize(JsonParser jp, DeserializationContext ctxt)
            throws IOException {
        ObjectCodec oc = jp.getCodec();
        IntNode node = oc.readTree(jp);
        int genderString = node.numberValue().intValue();

        return ResignType.of(genderString);
    }
}