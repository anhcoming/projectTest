...
public class AttitudeDeserializer extends JsonDeserializer<Attitude> {

    @Override
    public Attitude deserialize(JsonParser jp, DeserializationContext ctxt)
        throws IOException {
        ObjectCodec oc = jp.getCodec();
        IntNode node = oc.readTree(jp);
        int genderString = node.numberValue().intValue();

        return Attitude.of(genderString);
    }
}
