...
public class ContractDurationDeserializer extends JsonDeserializer<ContractDuration> {

    @Override
    public ContractDuration deserialize(JsonParser jp, DeserializationContext ctxt)
        throws IOException {
        ObjectCodec oc = jp.getCodec();
        IntNode node = oc.readTree(jp);
        int genderString = node.numberValue().intValue();

        return ContractDuration.of(genderString);
    }
}
