...
public class ResignStatusDeserializer extends JsonDeserializer<ResignStatus> {

    @Override
    public ResignStatus deserialize(JsonParser jp, DeserializationContext ctxt)
        throws IOException {
        ObjectCodec oc = jp.getCodec();
        IntNode node = oc.readTree(jp);
        int genderString = node.numberValue().intValue();

        return ResignStatus.of(genderString);
    }
}
