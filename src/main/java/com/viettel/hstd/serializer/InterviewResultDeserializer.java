...
public class InterviewResultDeserializer extends JsonDeserializer<InterviewResult> {

    @Override
    public InterviewResult deserialize(JsonParser jp, DeserializationContext ctxt)
            throws IOException {
        ObjectCodec oc = jp.getCodec();
        IntNode node = oc.readTree(jp);
        int genderString = node.numberValue().intValue();

        return InterviewResult.of(genderString);
    }
}